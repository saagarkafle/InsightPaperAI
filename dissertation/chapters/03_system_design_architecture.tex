...
\item Phase 3 — System Architecture and Pipeline Design (Weeks 2–3): \gls{mvc} architectural design, \gls{rag} pipeline module specification, and Pinecone index schema definition.

\item Phase 4 — Iterative Implementation and Fine-Tuning (Weeks 3–4): Incremental Streamlit prototype development, \gls{pdf} parsing integration, vector indexing, Groq \gls{api} middleware, and embedding model fine-tuning execution.

\item Phase 5 — Evaluation and Benchmarking (Week 5): Corpus-wide retrieval benchmarking, \gls{llm} comparative evaluation, and latency profiling.

\end{itemize}

Below as shown in Figure \ref{fig:sdlc} is a diagram illustrating the iterative \gls{sdlc} roadmap adopted for the development of InsightPaper AI.

\begin{figure}[H]

\includegraphics[width=0.9\linewidth]{figures/3.1.png}

\caption{\gls{sdlc} Iterative Prototyping Roadmap for InsightPaper AI}
...
3. Architecture Design & \gls{mvc} design, pipeline specification, schema design & Architecture diagrams & Week 2–3 \\

\hline

4. Implementation \& Fine-Tuning & Streamlit \gls{ui}, \gls{pdf} parsing, \gls{rag} pipeline, fine-tuning & Functional web app, fine\_tuned\_embedder/ & Week 3–4 \\

\hline

5. Evaluation \& Benchmarking & Retrieval benchmarking, \gls{llm} comparison, latency tests & Benchmark results report & Week 5 \\
...
FR-01 & The system shall accept \gls{pdf} research papers as input via a web interface & High \\

\hline

FR-02 & The system shall extract and clean all text content and figures from uploaded \gls{pdf}s & High \\
...
FR-07 & The system shall accept CSV/\gls{json} evaluation datasets and compute Token F1, Semantic Similarity, and \gls{llm}-as-a-Judge scores & Medium \\
...
NFR-03 & The system shall handle multi-page academic \gls{pdf}s reliably & Tested on 50+ page documents \\
...
NFR-05 & \gls{api} credentials shall be stored securely and never hardcoded & .env file configuration \\
...
UC-01 & Upload \gls{pdf} Research Paper & Researcher & Upload a \gls{pdf}; system parses, chunks, and indexes it into Pinecone \\

\hline

UC-02 & Select \gls{llm} Model & Researcher & Choose Qwen 3.6 27B or LLaMA 3.1 8B from a dropdown \\
...
UC-06 & Upload Evaluation Dataset & Evaluator & Upload a CSV or \gls{json} file with ground-truth question-answer pairs \\

\hline

UC-07 & Run Benchmark Evaluation & Evaluator & Trigger automated scoring across Token F1, Semantic Similarity, and \gls{llm}-as-a-Judge \\
...
\subsection{High-Level \gls{mvc} Architecture}

InsightPaper AI is structured following the \gls{mvc} architectural design pattern. This separation of concerns ensures that data logic, presentation logic, and orchestration logic remain independently modifiable, reducing coupling and improving maintainability across the codebase.
...
\newline \newline The Model layer, implemented in src/mvc/model.py, contains all business logic: \gls{pdf} parsing, text chunking, vector indexing via Pinecone, \gls{llm} inference via Groq, session state persistence, and evaluation metrics computation. The application entry point is app.py, a twelve-line file responsible solely for initialising the Streamlit browser tab configuration and instantiating the AppController.

\begin{figure}[H]

\includegraphics[width=0.9\linewidth]{figures/3.3.png}

\caption{High-Level \gls{mvc} System Architecture of InsightPaper AI}

\label{fig:mvc}

\end{figure}

\subsection{\gls{rag} Query Processing Sequence}
...
\item The \gls{rag} Pipeline in src/rag\_pipeline.py encodes the query into a 384-dimensional dense vector using the fine-tuned SentenceTransformer embedder loaded from models/fine\_tuned\_embedder/.
...
\item The Prompt Builder assembles the five retrieved context chunks into a strict system prompt instructing the \gls{llm} to answer solely from the provided content and never from its training data.

\item The Groq \gls{api} Client in src/llm\_qa.py dispatches the constructed prompt to the user-selected \gls{llm}. For Qwen 3.6, internal think reasoning tokens are stripped via regex parsing before the answer is returned.
...
\caption{\gls{uml} Sequence Diagram — \gls{rag} Query Processing Pipeline}
...
\gls{pdf} Parsing & PyMuPDF (fitz) & 1.24.0+ & C-accelerated \gls{pdf} processing providing robust text block extraction, figure detection, and caption heuristic matching. \\
...
Vector Database & Pinecone & 3.0.0+ & Serverless cloud-native \gls{hnsw} vector store with cosine similarity indexing and sub-50ms retrieval at scale. \\

\hline

\gls{llm} Inference & Groq \gls{api} & OpenAI-compatible & LPU-accelerated inference delivering 800+ tokens per second, significantly outperforming standard \gls{gpu}-based \gls{api}s for real-time \gls{qa}. \\

\hline

Primary \gls{llm} & Qwen 3.6 27B & qwen3.6-27b & 27-billion parameter model. Selected as default for Qwen 3.6 27B (Deep Analysis) for complex queries and 300–500 word executive summaries. \\

\hline

Secondary \gls{llm} & LLaMA 3.1 8B & llama-3.1-8b-instant & Lightweight 8B model. Selected for LLaMA 3.1 8B (Fast Inference) delivering ~400ms average latency for rapid iterative Q\&A. \\

\hline

Evaluation & Custom evaluation.py & — & Token F1, Semantic Similarity, and \gls{llm}-as-a-Judge implemented natively in src/evaluation.py. \\
...
metadata.text & String & Raw chunk text; injected directly as context into the \gls{rag} prompt \\

\hline

metadata.page & Integer & Source page number; displayed as an inline citation in the \gls{ui} \\
...
The evaluation framework in src/evaluation.py measures generated answer quality across three complementary dimensions, addressing the known limitations of single-metric assessment in \gls{qa} evaluation. Table \ref{tab:metrics} presents the metrics matrix.
...
\gls{llm}-as-a-Judge \citep{zheng2023judging} & Holistic & Faithfulness, Relevance, and Completeness rated 1–5 & Evaluator \gls{llm} judges generated answer against retrieved source context \\
...
Token F1 penalises semantically correct but paraphrased answers. Semantic Similarity addresses this by measuring meaning-level alignment. \gls{llm}-as-a-Judge captures subjective qualitative properties such as explanation depth and completeness that neither lexical nor embedding-based metrics can quantify.

\section{Chapter Summary}

This chapter presented the complete system design of InsightPaper AI. An iterative \gls{sdlc} methodology was selected to accommodate the empirical nature of \gls{rag} pipeline tuning. A formal requirements specification covering eight functional and five non-functional requirements was produced alongside a \gls{uml} use case model. The \gls{mvc} architectural pattern was adopted to ensure clean separation of \gls{ui}, orchestration, and data logic. Each technology was justified through explicit trade-off analysis. The \gls{pdf} parsing pipeline, overlapping chunking strategy, Pinecone vector schema, and three-dimensional evaluation framework were all specified in design terms. These design decisions are realised in code and demonstrated through prototypes in Chapter 4.

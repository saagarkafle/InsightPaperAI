...
\newline \newline Initial experimentation using the unmodified sentence-transformers/all-MiniLM-L6-v2 base model on the 30 held-out evaluation papers yielded a Recall@5 of only 23.3\% and a MRR of 0.1161. This demonstrated that the general-domain base embedder was systematically failing to distinguish between semantically similar but topically different academic paper chunks. Domain-specific fine-tuning was therefore identified as a necessary research intervention.
...
\subsection{Loss Function: MNRL}

MNRL \citep{henderson2017efficient, reimers2019sentence} is a contrastive learning objective function that leverages in-batch negatives to train dense retrieval models efficiently without requiring manually labelled negative examples.
...
\caption{In-Batch Negative Contrastive Learning Mechanics (MNRL)}
...
Loss Function & MNRL & Efficient contrastive learning with in-batch negatives \\
...
\newline \newline Recall@K measures the proportion of queries for which at least one relevant chunk appeared within the top-K results, with K set to 5 as the primary benchmark. MRR computes the average of the reciprocal rank of the first correct result across all queries, where an MRR of 1.0 indicates the correct chunk was ranked first for every query.

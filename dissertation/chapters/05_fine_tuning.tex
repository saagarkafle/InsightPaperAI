...
The embedding model is the foundational retrieval component of the InsightPaper AI \gls{rag} pipeline. It is responsible for mapping both document chunks and user queries into a shared dense vector space, where semantic similarity can be measured via cosine distance. The accuracy of downstream question-answering is directly bounded by retrieval precision: if the correct context chunks are not retrieved, no amount of \gls{llm} capability can compensate for missing information.
\newline \newline Initial experimentation using the unmodified sentence-transformers/all-MiniLM-L6-v2 base model on the 30 held-out evaluation papers yielded a Recall@5 of only 23.3\% and a \gls{mrr} of 0.1161. This demonstrated that the general-domain base embedder was systematically failing to distinguish between semantically similar but topically different academic paper chunks. Domain-specific fine-tuning was therefore identified as a necessary research intervention.
...
\newline \newline Fine-Tuning Compatibility: The model's 6-layer architecture supports efficient gradient updates with smaller batch sizes, making domain-adaptive contrastive fine-tuning tractable without requiring multi-\gls{gpu} hardware.
...
\caption{In-Batch Negative Contrastive Learning Mechanics (\gls{mnrl})}
...
Loss Function & \gls{mnrl} & Efficient contrastive learning with in-batch negatives \\
...
Fine-tuning was executed via scripts/finetune\_embedder.py. The script loads the training pairs from data/kaggle\_train.jsonl, initialises the base all-MiniLM-L6-v2 model, configures the \gls{mnrl} loss, and runs the training loop for 3 epochs. Upon completion, the fine-tuned model weights are saved to models/fine\_tuned\_embedder/. When InsightPaper AI is subsequently launched, src/rag\_pipeline.py detects the presence of this directory and automatically loads the fine-tuned embedder in preference to the base model.

\section{Model Evaluation: Recall@5 and \gls{mrr} Benchmark}
...
\newline \newline Recall@K measures the proportion of queries for which at least one relevant chunk appeared within the top-K results, with K set to 5 as the primary benchmark. \gls{mrr} computes the average of the reciprocal rank of the first correct result across all queries, where an \gls{mrr} of 1.0 indicates the correct chunk was ranked first for every query.
...
\textbf{Model Variant} & \textbf{Recall@5} & \textbf{\gls{mrr}} & \textbf{Correct in Top-5} & \textbf{Latency} \\
...
Below as shown in Figure \ref{fig:recall} is a comparative bar chart illustrating the Recall@5 and \gls{mrr} performance comparison between the base and fine-tuned models.

\begin{figure}[H]

\includegraphics[width=0.9\linewidth]{figures/5.3.png}

\caption{Recall@5 and \gls{mrr} Performance Comparison — Base vs Fine-Tuned Embedder}

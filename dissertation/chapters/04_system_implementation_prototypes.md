...
The AppController.run() method checks st.session\_state for an active processed paper. If no paper is loaded, the landing page view is rendered. If a paper session exists, the main dashboard is rendered with the appropriate tabs.
...
During indexing, text chunks are produced by the sliding-window chunker and passed individually to the SentenceTransformer embedder. The resulting 384-dimensional vectors are batched and upserted into Pinecone alongside their metadata payloads. Pinecone connections are established using the PINECONE\_API\_KEY loaded via python-dotenv from the .env file.

During retrieval, the query string is encoded by the same embedder and submitted to Pinecone's query API with top\_k set to 5 and a filter clause scoped to the active paper\_id namespace. The returned matches contain both the vector score and the associated metadata.text and metadata.page fields used for citation display.
...
The following subsections present annotated screenshots of each major interface prototype. All prototypes were implemented in Streamlit with a custom CSS stylesheet injected via src/ui/css.py, featuring a cohesive dark theme design system with structured card containers, distinct section panels, accent buttons, and metadata highlight tags.
...
The overview card includes a dedicated Read Elaborated Summary (300–500 words) button, allowing users to launch an in-depth modal dialog. Below as shown in Figure \ref{fig:proto2} is the main dashboard summary interface.
...
The modal window features a dark header card displaying the paper title, academic field tag, and difficulty badge. The summary text is parsed into four distinct executive breakdown cards: Core Problem \& Research Context, Proposed Methodology \& Key Innovations, Critical Findings \& Benchmarks, and Broader Impact \& Future Directions. Each section card is styled with a prominent left border and highlighted section headers. A stats footer displays total word count, 100\% grounding verification, and the active LLM engine. Below as shown in Figure \ref{fig:proto4} is the Detailed Executive Summary Modal interface.
...
The key components visible in this interface are: the modal header banner with paper title and metadata tags; the four structured executive breakdown cards with distinct left accent borders; the icon-anchored section titles; the scrollable dark summary container; and the executive stats footer displaying word count, grounding status, and active LLM model.
...
After validation, the dataset text content is extracted and indexed into Pinecone under a dataset source namespace, enabling semantic search over dataset content separately from the PDF body.
